WMS.module("Clients.Orders", function(Orders, WMS, Backbone, Marionette, $, _) {
  ...
  var EstimateRows = Marionette.CompositeView.extend({
    ...
    templateHelpers: {
      cols: 6,
      headers: [
        "Cod. Articolo", "Descrizione", "U.M.", "Prezzo", "Sconto %", "Q.ta", ""
      ]
    },
    tagName             : "tr",
    className           : "drilldown",
  ...
  var Estimate = Views.TableRowView.extend({
    behaviors: [{
      ...
      className     : "info",
      condition     : { accepted: false },
      failureMsg    : "Preventivo già approvato."
    }, {
  ...
  var OrderRowForm = Views.ModalFormView.extend({
    ...
    behaviors: [{
      ...
      insertPoint   : "div.modal-body",
      skipFields    : ["total"],
      modelClass    : WMS.Models.ClientOrderRow
    }, {
      behaviorClass: Behaviors.CascadingDefaultBehavior,
  ...
  Views.Rows = Views.PanelTableView.extend({
    title: "Righe Ordine",
    childView: Row,
    childViewEventPrefix: "client:order:row",
    modelClass    : WMS.Models.ClientOrderRow,
    headers       : [
  ...
  Views.SelectCommission = WMS.Common.Views.ModalFormView.extend({
    initialize: function(options) {
      this.model = new WMS.Models.ClientOrderCommission(_.pick(options, "clientId"));
